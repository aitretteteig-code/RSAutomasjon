...
const runId = process.env.HC003_RUN_ID || new Date().toISOString().replace(/\D/g, '').slice(0, 14);
const recipeId = process.env.HC003_RECIPE_ID || '757709';
const recipeName = process.env.HC003_RECIPE_NAME || 'LAM YTREFILET MARINERT PR KG';
const existingProductionId = process.env.HC003_PRODUCTION_ID || '';
const evidenceDir = path.resolve('test-artifacts/evidence');
const screenshot = (name) => path.join(evidenceDir, `HC-003-${runId}-${name}.png`);
const resultsPath = path.join(evidenceDir, `HC-003-${runId}-results.json`);
...
async function waitForProductionDetailsAfterStart(page) {
  await page.waitForURL(/\/batches\/production\/details\/\d+/i, { timeout: 45000 }).catch(() => {});
  await page.waitForLoadState('networkidle').catch(() => {});
  await page.waitForTimeout(8000);

  const detailsVisible = await page
    .locator('body')
    .filter({ hasText: /Produksjon|Plukker ingredienser|Kalkulering/i })
    .first()
...
async function openOrCreateProduction(page, articleId) {
  ...
  await waitForProductionDetailsAfterStart(page);
...
async function createAndVerifyKgProduction(page, articleId) {
...
test.describe('HC-003 Produksjon av oppskrift KG', () => {
  test('produser KG-oppskrift og verifiser pris og deklarasjon i RS Store', async ({ page }) => {
    test.setTimeout(90 * 60 * 1000);
    fs.mkdirSync(evidenceDir, { recursive: true });
    await loginIfNeeded(page);
    await ensureStoreSelected(page, process.env.RS_STORE_NAME || DEFAULT_STORE_NAME);

    const production = await createAndVerifyKgProduction(page, recipeId);
    ...
    const result = {
      ...
      production,
      manualAction,
      evidencePrefix: `test-artifacts/evidence/HC-003-${runId}-`,
    ...
    console.log(`HC-003-RESULT ${JSON.stringify(result, null, 2)}`);
